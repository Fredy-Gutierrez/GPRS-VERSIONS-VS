...
{
    public partial class FormSocketTcpServer : Form
    {
        Configurations c;
        ...
        TcpServerMessagesModel tcpServerMessagesModel = new TcpServerMessagesModel();

        public FormSocketTcpServer(FormServidor formServidor)
        {
            InitializeComponent();
        ...
        private void FormSocketTcpServer_Load(object sender, EventArgs e)
        {

            XmlNodeList listaClients = c._ReadXml("Servers", "Tcp", "TcpServer", "Server");
            ...
            if (!lblName.Text.Equals("Nombre"))
            {
                if (!Session.user.Equals(string.Empty))
                {
                    if (btnSwitchOnOff.IsOn)
                    {
                        ...
                        driverMaster.beginOneTcpServerServer(lblName.Text, txtPort.Text);
                    ...
                    else
                    {
                        ...
                        driverMaster.closeOneTcpServerS(lblName.Text);
        ...
        private void FormSocketTcpServer_FormClosing(object sender, FormClosingEventArgs e)
        {
            formServidor.listTcpServerForm.Remove(this);
        ...
        private void chargeBeforeMessage()
        {
            try
            {
                ...
                DataTable t = tcpServerMessagesModel.readMessages(lblName.Text, "Servers", msg);
                ...
                for (int i = 0; i < t.Rows.Count; i++)
                {
                    ...
                    {
                        msg = "TcpServer " + t.Rows[i][2].ToString() + ": " + t.Rows[i][1].ToString() + "    [" + t.Rows[i][4].ToString() + "   " + t.Rows[i][5].ToString() + "]";
                    }
                    else
                    {
                        msg += "\n" + "TcpServer " + t.Rows[i][2].ToString() + ": " + t.Rows[i][1].ToString() + "    [" + t.Rows[i][4].ToString() + "   " + t.Rows[i][5].ToString() + "]";
                    }
                }
